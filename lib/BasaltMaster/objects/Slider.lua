...
return function(name, basalt)
    ...
    local barType = self:getBarType()
        ...
        for i = 0, size do
            if ((barType == "vertical" and oby + i == y) or (barType == "horizontal" and obx + i == x)) and (obx <= x) and (obx + w > x) and (oby <= y) and (oby + h > y) then
                ...
                self:setValue(maxValue / (size-1) * (index-1))
                self:updateDraw()
            end
        end
    end

    base:onChange(function (self,e,value)
        local w,h = self:getSize()
        local size =  self:getBarType()== "vertical" and h or w
        local maxValue = self:getMaxValue()
        value = value or 0
        index = math.floor((size-1)/maxValue * value +1+0.5)
        self:updateDraw()
        basalt:debug(index)
    end)
    ...
    local object = {
        ...
        setIndex = function(self, _index)
            ...
            self:setValue(maxValue / ((barType == "vertical" and h or w)-1) * (index-1))
            self:updateDraw()
            return self
        end,
                    ...
                    for n = 0, h - 1 do
                        if (index == n + 1) then
                            ...
                            if (n + 1 < index) or (n + 1 > index - 1 + symbolSize) then
                                self:addBlit(1, 1+n, "\127", tHex[fgCol], tHex[symbolBG])
                            end
                        end
                    end
                end
