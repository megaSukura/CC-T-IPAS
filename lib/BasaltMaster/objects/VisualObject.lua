...
return function(name, basalt)
    ...
    base:addProperty("Background", "color|any", colors.black)
    ...
    base:addProperty("Width", "number", 1, false, function(self, val)
        ...
        if (parent ~= nil and val~=width) then
    ...
    base:addProperty("Height", "number", 1, false, function(self, val)
        local width = self:getProperty("Width")
        local Height = self:getProperty("Height")
        if (parent ~= nil and Height~=val) then
    ...
    base:addProperty("BaseDraw", "boolean", true)
    ...
    local object = {
        ...
        draw = function(self)
            self:addDraw("base", function()
                if (self:getBaseDraw()==false)then
                    return
                end
                local w,h = self:getSize()
